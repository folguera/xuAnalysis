...
inc = [None,'JESUp','JESDown','JERUp','JERDown']
level = {lev.tight:'tight'}

for l in level.keys():
 for i in range(0,len(inc)):
  Da=[];Back=[]; WZ=[]; TT=[]; DY=[]; VV=[]; WZ=[]; Da=[]; Sig=[]; Back=[]; ZZ=[]; WW=[];
  for pr in process:
   PR.append(pr)
   for c in chan.keys():
    if pr=='WZTo3LNU': 
     ...
     DY.append(GetYield2(path, pr, chan[c], level=level[l], inc = inc[i], var = 'Yields')*Lumi)
    elif pr=='WWTo2L2Nu':
     WW.append(GetYield2(path, pr, chan[c], level=level[l], inc = inc[i], var = 'Yields')*Lumi)
    elif pr=='ZZTo2L2Nu' or pr=='ZZTo4L':
     ZZ.append(GetYield2(path, pr, chan[c], level=level[l], inc = inc[i], var = 'Yields')*Lumi)
    elif pr=='tW_noFullHad' or pr=='tbarW_noFullHad' or pr=='TT':
     TT.append(GetYield2(path, pr, chan[c], level=level[l], inc = inc[i], var = 'Yields')*Lumi)
