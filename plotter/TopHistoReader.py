...
#from WZ analysis
level = {0:'lep', 1:'met', 2:'wpt', 3:'zpt', 4:'m3l', 5:'htmiss',6:'sr',7:'srtight',8:'tight'}
...
class Process:
  ...
  def SetYieldFid(self, y):
    self.y = y

  def SetYieldTot(self, y):
    self.y = y
  ...
  def __init__(self, name, color = 1, samples = '', isData = False, isSignal = False, Yield = 0, systName = '', NormUnc = 0, SystUnc = -999, StatUnc = 0, ExpUnc = {}, ModUnc = {}):
    ...
    self.syst = {}
    self.SetYield(Yield)
    self.SetYieldFid(Yield)
    self.SetYieldTot(Yield)
    self.SetSystName('')
